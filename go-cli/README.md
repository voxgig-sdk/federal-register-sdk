...
boru-driven command-line client **and** interactive REPL for the FederalRegister
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/federal-register-cli)
make build

# 2. See usage (words, entities, env vars)
./federal-register-cli --help
...
./federal-register-cli load 1 document            # {id:1} shorthand
./federal-register-cli load '{id:1}' document       # explicit match map

# 5. Override the API base URL for a single call
FEDERAL_REGISTER_BASE=https://api.example.com ./federal-register-cli list document

# 6. No arguments -> interactive REPL
./federal-register-cli
federal-register> list document
federal-register> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/federal-register-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/federal-register-cli list document
   ```
...
```sh
./federal-register-cli load 1 document          # scalar shorthand for {id:1}
./federal-register-cli load '{id:1}' document     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `federal-register>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `document`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/federal-register-cli`. |
...
(`./federal-register-cli <expr>`) and interactively (the REPL), and expressions compose the
