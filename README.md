...
The Federal Register API provides access to data from the Federal Register, allowing users to fetch single or multiple documents, perform searches, and access metadata associated with these documents.

Learn more about Federal Register API at [federalregister.gov](https://federalregister.gov).
...
This SDK exposes the API as a small set of **semantic entities** — Document — that you
...
const items = await client.Document().list()
```
...
```go
client := sdk.Test()
result, err := client.Document(nil).List(
    nil, nil,
)
```
...
$documents = $client->Document()->list();
print_r(array_map(fn($item) => $item->data_get(), $documents));
