...
[MCP](https://modelcontextprotocol.io) server exposing the FederalRegister SDK as
two agent tools — `federal-register_list` and `federal-register_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/federal-register-mcp)
make build

# 2. Provide credentials via the environment
export FEDERAL_REGISTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user federal-register \
  -- /absolute/path/to/federal-register-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./federal-register-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// federal-register_list: first page of records
{ "entity": "document" }
{ "entity": "document", "query": { } }

// federal-register_load: one record by id
{ "entity": "document", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/federal-register-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user federal-register \
     -- "$PWD"/dist/*/federal-register-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `federal-register_list` and `federal-register_load` tools now appear
   in new sessions. Ask the agent to *"list document using federal-register"*
   and it calls `federal-register_list` with `{"entity":"document"}`.
...
```sh
export FEDERAL_REGISTER_APIKEY=sk_live_xxx            # API key
export FEDERAL_REGISTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `federal-register_list` tool
...
### Call the `federal-register_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `federal-register_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `federal-register_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FEDERAL_REGISTER_APIKEY` | API key sent with every request. |
| `FEDERAL_REGISTER_BASE` | Optional override of the API base URL. |
...
```sh
./federal-register-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"federal-register_load","arguments":{"entity":"document","query":{"id":1}}}}'
```
