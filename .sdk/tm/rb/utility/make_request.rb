...
module FederalRegisterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FederalRegisterResponse.new({})
    ...
    if fetch_err
      ...
      response = FederalRegisterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FederalRegisterResponse.new(fetched)
    else
