...
// FederalRegister SDK feature test
//
...
class FtCtrl extends FederalRegisterControl
{
...
class FtClient
{
    ...
    private FederalRegisterUtility $utility;

    public function __construct(FederalRegisterUtility $utility, array $options)
    {
    ...
    public function get_utility(): FederalRegisterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FederalRegisterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FederalRegisterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FederalRegisterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FederalRegisterConfig::make_feature($name);
    ...
    private static function buildUrl(FederalRegisterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FederalRegisterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FederalRegisterResponse($fetched);
        }
    ...
    private function fail(FederalRegisterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FederalRegisterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
