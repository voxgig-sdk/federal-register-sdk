...
// FederalRegister SDK retry feature
...
class FederalRegisterRetryFeature extends FederalRegisterBaseFeature
{
    ...
    public function init(FederalRegisterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FederalRegisterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FederalRegisterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FederalRegisterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
