...
// FederalRegister SDK Document entity
...
class DocumentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DocumentEntity($this->_client, $opts);
    }

    /**
     * @param Document|array $args Document data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FederalRegisterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Document|array The current Document data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Document fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FederalRegisterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Document fields).
     */
     ...
     *
     * @param DocumentLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed DocumentLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Document|array The loaded Document as an assoc-array at the
     *   SDK boundary; throws FederalRegisterError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FederalRegisterHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param DocumentListMatch|array|null $reqmatch Match filter (any subset
     *   of Document fields) as an assoc-array; DocumentListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Document[]|array A list of Document items as assoc-arrays at
     *   the SDK boundary; throws FederalRegisterError on failure (item-5 convention).
     */
