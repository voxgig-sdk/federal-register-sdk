...
import (
	...
	omni "github.com/voxgig-sdk/federal-register-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FederalRegisterError); is {
				out := map[string]any{
					"name":    "FederalRegisterError",
...
func omniCtx(arg any, client *sdk.FederalRegisterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FederalRegisterSDK); is {
			client = live
