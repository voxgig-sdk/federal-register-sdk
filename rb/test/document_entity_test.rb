...
class DocumentEntityTest < Minitest::Test
  def test_create_instance
    testsdk = FederalRegisterSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = FederalRegisterSDK.test(seed, nil)
    seen = base.Document(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = FederalRegisterConfig.make_config
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FEDERAL_REGISTER_TEST_DOCUMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    document_ref01_data = nil
    if document_ref01_data_raw.length > 0
...
def document_basic_setup(extra)
  ...
  entid_env_raw = ENV["FEDERAL_REGISTER_TEST_DOCUMENT_ENTID"]
  ...
  if env["FEDERAL_REGISTER_TEST_LIVE"] == "TRUE"
    ...
    client = FederalRegisterSDK.new(Helpers.to_map(merged_opts))
  end
