# FederalRegister Ruby SDK



The Ruby SDK for the FederalRegister API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Document` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = FederalRegisterSDK.test(testopts, sdkopts)
```
...
raise a `FederalRegisterError` (a `StandardError` subclass), so wrap
...
```
rb/
├── FederalRegister_sdk.rb       -- Main SDK module
...
The main module (`FederalRegister_sdk`) exports the SDK class
