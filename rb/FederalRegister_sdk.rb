...
require_relative 'FederalRegister_types'
...
class FederalRegisterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FederalRegisterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FederalRegisterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FederalRegisterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FederalRegisterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FederalRegisterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FederalRegisterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FederalRegisterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FederalRegisterError.new(
        "#{op}_allow",
        "FederalRegisterSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FederalRegisterError => err
    ...
    if fetched.is_a?(Hash)
      status = FederalRegisterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FederalRegisterError.new(
        "graphql_error", "FederalRegisterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FederalRegisterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
