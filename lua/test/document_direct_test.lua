...
describe("DocumentDirect", function()
  it("should direct-list-document", function()
    local setup = document_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-document", setup.live and "live" or "unit")
  ...
  it("should direct-load-document", function()
    local setup = document_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-document", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function document_direct_setup(mockres)
