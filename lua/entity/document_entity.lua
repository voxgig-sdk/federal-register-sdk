...
DocumentEntity.__index = DocumentEntity


function DocumentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DocumentEntity)
  self._name = "document"
  self._client = client
...
function DocumentEntity:get_name()
  return self._name
end


function DocumentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DocumentEntity.new(self._client, opts)
end
...
function DocumentEntity:mark_deleted()
  self._deleted = true
end


function DocumentEntity:deleted()
  return true == self._deleted
end


function DocumentEntity:data_set(args)
  if args ~= nil then
...
function DocumentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DocumentEntity:match_set(args)
  if args ~= nil then
...
function DocumentEntity:match_get()
...
function DocumentEntity:stream(action, args, callopts)
...
---@param reqmatch DocumentLoadMatch
---@param ctrl? table
---@return Document
---@return string? err
function DocumentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DocumentListMatch
---@param ctrl? table
---@return Document[]
---@return string? err
function DocumentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DocumentEntity:_run_op(ctx, post_done)
  local utility = self._utility
