# FederalRegister Lua SDK



The Lua SDK for the FederalRegister API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Document()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/federal-register-sdk/releases)),
...
local result, err = client:Document():list()
...
| `agencies` | `table` | Agencies associated with the document |
...
| `topics` | `table` | Topics associated with the document |
...
```
lua/
├── federal-register_sdk.lua    -- Main SDK module
...
The main module (`federal-register_sdk`) exports the SDK constructor
...
```lua
local document = client:Document()
document:list()

-- document:data_get() now returns the document data from the last list
-- document:match_get() returns the last match criteria
```
