# FederalRegister Lua SDK Reference

Complete API reference for the FederalRegister Lua SDK.
...
```lua
local results, err = client:Document(nil):list(nil, nil)
```
...
```lua
local result, err = client:Document(nil):load({ id = "document_id" }, nil)
```
