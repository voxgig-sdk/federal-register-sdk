# FederalRegister TypeScript SDK



The TypeScript SDK for the FederalRegister API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Document()` — each with a small set of operations (`list`, `load`)
...
```ts
try {
  const document = await client.Document().load({ id: 'example_id' })
  console.log(document)
} catch (err) {
...
| `tester(testopts?, sdkopts?)` | `FederalRegisterSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FederalRegisterSDK.test(testopts?, sdkopts?)` | `FederalRegisterSDK` | Create a test-mode client. |
...
| `client` | `client(): FederalRegisterSDK` | Return the parent SDK client. |
...
federal-register/
├── src/
│   ├── FederalRegisterSDK.ts        # Main SDK class
