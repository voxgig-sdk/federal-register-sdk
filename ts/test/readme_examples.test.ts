...
//     (\`new FederalRegisterSDK(...)\` / \`FederalRegisterSDK.test(...)\` become
//     \`FederalRegisterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FederalRegisterSDK'
...
const TEST_SEED = {"entity":{"document":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FederalRegisterSDK(...)\` and any
// \`FederalRegisterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FederalRegisterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FederalRegisterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
