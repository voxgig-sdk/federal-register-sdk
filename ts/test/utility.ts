...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FEDERALREGISTER_TEST_LIVE ||
    'TRUE' === process.env.FEDERALREGISTER_TEST_OVERRIDE
  ) {
  ...
  m.FEDERALREGISTER_TEST_EXPLAIN = process.env.FEDERALREGISTER_TEST_EXPLAIN || m.FEDERALREGISTER_TEST_EXPLAIN
