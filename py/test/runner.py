# FederalRegister SDK test runner
...
class FederalRegisterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FederalRegisterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FederalRegisterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FederalRegisterTestRunner.getenv("FEDERAL_REGISTER_TEST_LIVE")
        override = FederalRegisterTestRunner.getenv("FEDERAL_REGISTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FederalRegisterTestRunner.getenv(key)
        ...
        explain = FederalRegisterTestRunner.getenv("FEDERAL_REGISTER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FEDERAL_REGISTER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FederalRegisterTestRunner._test_control is not None:
            return FederalRegisterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FederalRegisterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FederalRegisterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FederalRegisterTestRunner.load_test_control()
...
def load_env_local():
    FederalRegisterTestRunner.load_env_local()


def env_override(m):
    return FederalRegisterTestRunner.env_override(m)


def entity_data(v):
    return FederalRegisterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FederalRegisterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FederalRegisterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FederalRegisterTestRunner.load_test_control()


def live_client_options():
    return FederalRegisterTestRunner.live_client_options()


def live_delay_ms():
    return FederalRegisterTestRunner.live_delay_ms()
