...
class TestDocumentEntity:
    ...
    def test_should_stream(self):
        ...
        base = FederalRegisterSDK.test(seed, None)
        seen = list(base.Document(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Document(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _document_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "document." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FEDERAL_REGISTER_TEST_DOCUMENT_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        document_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.document")))
        document_ref01_data = None
        if len(document_ref01_data_raw) > 0:
...
def _document_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/document/DocumentTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FEDERAL_REGISTER_TEST_DOCUMENT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FEDERAL_REGISTER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = FederalRegisterSDK(helpers.to_map(merged_opts))

    _live = env.get("FEDERAL_REGISTER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FEDERAL_REGISTER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
